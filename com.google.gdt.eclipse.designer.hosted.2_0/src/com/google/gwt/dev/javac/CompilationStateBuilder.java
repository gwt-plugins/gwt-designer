...
import com.google.gwt.dev.javac.CompilationUnitBuilder.CachedResourceCompilationUnitBuilder;
...
public class CompilationStateBuilder {
  ...
  public class CompileMoreLater {
    ...
    private final class UnitProcessorImpl implements UnitProcessor {
      ...
      public void process(CompilationUnitBuilder builder,
        ...
        // XXX Instantiations
        if (false && cud.compilationResult().hasErrors()) {
  ...
  public synchronized CompilationState doBuildFrom(TreeLogger logger,
    ...
    for (Resource resource : resources) {
	  ...
	  // try to get from the binary cache
      CompilationUnit unit = CompilationUnitDiskCache.get().get(resource, jsProgram);
      if (unit != null) {
    	  resultUnits.put(unit.getTypeName(), unit);
      }
    }
    ...
    for (Resource resource : resources) {
      ...
      // >>> XXX Instantiations: Eclipse 3.5 compiler doesn't like 'package-info.java' CUs, so exclude them
      ...
      if (validUnit != null) {
        ...
        // >>> XXX Instantiations
        builders.add(new CachedResourceCompilationUnitBuilder(typeName, resource));
        // <<< XXX Instantiations
      }
    }
