...
public class ResourceOracleImpl implements ResourceOracle {
  ...
  // XXX >>> Instantiations
  private static boolean isJreEntry(String urlString) {
	  return urlString.contains("/jre/") || urlString.contains("/System/Library");
  }
  // XXX <<< Instantiations
    ...
    for (; classLoader != null; classLoader = classLoader.getParent()) {
      if (classLoader instanceof URLClassLoader) {
        ...
        for (URL url : urls) {
          ...
          try {
       	  	// XXX >>> Instantiations
        	// probably bug in GWT: having gwt-user.jar somewhere in path with spaces, url.toURI() complains on invalid character   
  ...
  private static synchronized List<ClassPathEntry> getAllClassPathEntries(
    ...
    /* >>> Instantiations List<ClassPathEntry> classPath = classPathCache.get(classLoader); 
    if (classPath == null) {*/
      List<ClassPathEntry> classPath = new ArrayList<ClassPathEntry>();
      addAllClassPathEntries(logger, classLoader, classPath);
      /* >>> Instantiations classPathCache.put(classLoader, classPath);
    }*/
    return classPath;
  }
