...
public abstract class ModuleSpace implements ShellJavaScriptHost {
  ...
  public void dispose() {
    ...
    // XXX >>> Instantiations
    // Clear logger
    if (threadLocalLogger.get() == host.getLogger()) {
      threadLocalLogger.set(null);
    }
    // XXX <<< Instantiations
  }
  // XXX >>> Instantiations
  public static void setLogger(TreeLogger logger) {
    threadLocalLogger.set(logger);
  }
  private ClassLoader m_devClassLoader;
  public void setDevClassLoader(ClassLoader projectClassLoader) {
	  m_devClassLoader = projectClassLoader;
  }
  ...
  private <T> T rebindAndCreate0(String requestedClassName)
