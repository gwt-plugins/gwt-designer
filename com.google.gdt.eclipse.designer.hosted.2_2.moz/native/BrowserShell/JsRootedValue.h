...
{
public:
  // JSVAL_VOID is changed in 1.9.1
  static jsval JSVAL_VOID;
private:
...
protected:
  ...
  static void pushContext(JSContext* context) {
    ...
    tracer.log("pushed context=%p", context);
  ...
  static void popContext() {
    ...
    contextStack.pop();
    tracer.log("popped context=%p", context);
  }
...
public:
   ...
   */
  static void ensureRuntime(JSContext* cx) {
    if(!runtime) {
      runtime = JS_GetRuntime(cx);
      contextStack.push(cx);

      // mozilla guys sucks! hack on the hack and hack again!
      // since xulrunner 1.9.1 (firefox 3.5) the value of JSVAL_VOID changed, see https://bugzilla.mozilla.org/show_bug.cgi?id=432881 why.
      // so here is another hack: get the undefined property from global object and see which type it has.
      JSObject *obj = JS_GetGlobalObject(cx);
      jsval val = 0;
      if (obj == NULL || JS_GetProperty(cx, obj, "__wbp_fake_prop_E26388A0A15011DE8E7881EB55D89593", &val) == JS_FALSE) {
         fprintf(stderr, "Can't determine JSVAL_VOID value.\n");fflush(stderr);
         return;
      }
      // assing it to JSVAL_VOID, which was previously undef'ed.
      JSVAL_VOID = val;
    }
  }

  static bool isVoid(jsval val) {
    return JSVAL_VOID == val;
  }

  static bool isNumber(jsval val) {
    return ((val & JSVAL_INT) && !isVoid(val)) || (val & JSVAL_DOUBLE);
  }
  /*
