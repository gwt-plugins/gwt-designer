...
public class LowLevelWebKit {
  ...
  public static void cleanupWrappers(ClassLoader cl) {
    synchronized (m_trackingWrappers) {
      ...
      for (DispatchObject<?> refObj : m_trackingWrappers) {
        if (refObj instanceof WebKitDispatchAdapter) {
          ...
          WeakReference<ClassLoader> classLoaderRef = adapter.classLoaderRef;
          if (classLoaderRef.get() == null || classLoaderRef.get() == cl) {
            adapter.classLoaderRef.clear();
            adapter.dispIdOracleRef.clear();
